...
const make = Effect.gen(function* () {
  ...
  const dispatch: ThreadBootstrapShape["dispatch"] = (command, options) => {
    const dispatchFromClient: OrchestrationEngine.OrchestrationEngineShape["dispatch"] = (
      nestedCommand,
    ) => orchestrationEngine.dispatch(nestedCommand, options);
      ...
      const bootstrapProgram = Effect.gen(function* () {
        ...
        if (bootstrap?.createThread) {
          const created = yield* dispatchFromClient({
            ...
            ...(bootstrap.createThread.team === undefined
              ? {}
              : { team: bootstrap.createThread.team }),
  ...
  return { dispatch } satisfies ThreadBootstrapShape;
});
