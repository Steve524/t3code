...
// FORK: Identify Team Workflow report messages in the timeline.
import { selectTeamReport } from "@t3tools/client-runtime/state/team";
// FORK: Render Team Workflow reports with a fork-owned component.
import { TeamReportTimelineRow } from "../../fork/components/chat/TeamReportTimelineRow";
import {
...
const TimelineRowContent = memo(function TimelineRowContent({ row }: { row: TimelineRow }) {
  ...
  const teamReport = row.kind === "message" ? selectTeamReport(row.message) : null; // FORK: Recognize team reports.
  ...
  return (
    ...
    >
      ...
      {/* FORK-BEGIN: Render Team Workflow reports as collapsible rows. */}
      {row.kind === "message" && row.message.role === "user" ? (
        teamReport ? (
          <TeamReportTimelineRow reports={teamReport} />
        ) : (
