...
import { selectTeamReport, type TeamReportWorker } from "@t3tools/client-runtime/state/team";
import {
...
const TimelineRowContent = memo(function TimelineRowContent({ row }: { row: TimelineRow }) {
  ...
  const teamReport = row.kind === "message" ? selectTeamReport(row.message) : null;
...
function TeamReportTimelineRow({ reports }: { reports: ReadonlyArray<TeamReportWorker> }) {
  return (
